...
Node.js不是一种语言，是使用JavaScript进行服务器端应用程序开发的运行环境
...
与PHP比较：
PHP是一种语言，而Node.js是一种框架
两者都是开放源码，都是致力于web开发，并且都可用于相似的项目
PHP和Node.js都可以管理任何复杂的Web应用程序，但是它们是用不同的体系结构和概念构建的
...
-- 与Angular.js，jQuery，React.JS等前端技术一起使用
-- Node.js内置了处理网络请求和响应的函数库，所以不需要单独的服务器（Apache、Ngix）或者其他依赖


与Node.js中的编程相比，PHP中的编程要容易得多。
PHP中的编程风格更简单，不需要任何编译器或转换器，也没有托管限制，并且在PHP中使用SQL进行整个设置非常简单
当遇到诸如正常数据提取要求之类的小用例时，PHP可以快速启动并在短时间内使应用程序运行

而Node.js非常复杂
在Node.js中部署框架和应用程序并不容易，并且需要精心准备好服务器基础架构
PHP可与MySQL数据库完美配合，同时为PostgreSQL和MariaDB等多种数据库形式提供类似的支持
Node.js包含用于访问SQL的各种库。此外，它使用JSON附加到NoSQL数据库
...
Node.js有前途，并且肯定会占据PHP市场的一部分，但是不能完全取代 PHP
